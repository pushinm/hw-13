...
def show_teachers_grid(request, pk=None):
    if not pk:
        ...
        # annotate_ = Wage.objects.annotate('teacher')
        # agrigate_ = Wage.objects.agrigate('teacher')
        select_related_ = Wage.objects.get_queryset().select_related('teacher').get(pk=pk)
       # prefetch_related_ = Wage.objects.prefetch_related('teacher')

        context = {
           # 'annotate_': annotate_,
           # 'agrigate_': agrigate_,
            'current_teacher': select_related_,
           # 'prefetch_related_': prefetch_related_
        }
